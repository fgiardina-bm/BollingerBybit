         ...
         for symbol in simbolos:
            try:
                ...
                if float(posiciones['result']['list'][0]['size']) != 0:
                    ...
                    log_message = f"{symbol:<18} Price: {precio:<15.5f}\tp24h: {price24hPcnt:<3.2f}\t{str(precio >= data['UpperBand']):<5}\t{str(precio <= data['LowerBand']):<5}\tBB_W: {data['BB_Width_%']:<3.0f}\tRSI: {rsi:<3.0f}\tt1:{tendencia:<2}\tt2:{tendencia2:<2}\tBBW: {Bollinger_bands_width}\tmonitoring: {monitoring}"
                    logger(log_message)
