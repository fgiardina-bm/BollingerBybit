         ...
         for symbol in simbolos:
            try:
                ...
                if float(posiciones['result']['list'][0]['size']) != 0:
                    ...
                    log_message = f"{time.strftime('%Y-%m-%d %H:%M:%S')}\t{symbol:<15} Price: {precio:<12.5f}\tp24h: {price24hPcnt:<3.5f}\tFF: {fundingRate:<3.5f}\t{str(precio >= data['UpperBand']):<5}\t{str(precio <= data['LowerBand']):<5}\tBB_W: {data['BB_Width_%']:<5.0f}\tRSI: {rsi:<3.0f}\tcci: {cci:<5.0f}\tt1:{tendencia:<5}\tt2:{tendencia2:<5}"
                    logger(log_message)
