         ...
         for symbol in simbolos:
            try:
                ...
                if float(posiciones['result']['list'][0]['size']) != 0:
                    ...
                    log_message = f"{symbol:<18} Price: {precio:<15.5f}\tp24h: {price24hPcnt:<3.2f}\trsi: {rsi[-1]:.1f}\tbajista: {patron_confirmado_bajista}\talcista: {patron_confirmado_alcista}\ts:{cerca_soporte},r:{cerca_resistencia},v:{volumen_aumento} | bv:{buy_volume},sv:{sell_volume},dv:{delta_volume}"
                    logger(log_message)
